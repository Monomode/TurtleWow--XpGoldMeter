...
frame:SetScript("OnUpdate", function()
    ...
    if elapsedTime <= 0 then elapsedTime = 1 end  -- Prevent divide-by-zero (Added safeguard)
    ...
    frame.text:SetText(string.format(
        "XP/hour: %.0f\nGold/hour: %.2f\nLoot/hr: %.2f\nTotal XP: %d\nTotal Gold: %.2f\nLoot Value: %.2f\nTime: %s",
        xpPerHour, goldPerHour, lootPerHour, totalGainedXP, gainedGold, totalLootGold, timeString
    ))
end)
